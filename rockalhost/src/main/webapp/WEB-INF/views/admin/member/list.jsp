...
function onclickWithdrawal(){
	confirm("해당 회원님을 탈퇴처리 하시겠습니까?");
	location.href="${path}/admin/onclickWithdrawal/+${row.id}";
}
function list(curPage) {
	location.href="${path}/admin/memberlist.do?curPage="+curPage;
}

</script>
<style type="text/css">
#withdrawal_a a:link{color: red;}
#withdrawal_a a:visited{color: red;}
#withdrawal_a a:hover{font-weight: bold; color: red;}
#withdrawal_a{
color: red;
}
#search_div{
...
<h2>활동회원 리스트  <span style="font-size: 17px;"> &lt;총${memberCount}명&gt;</span></h2>
...
<c:forEach var="row" items="${list}">
  ...
  <td><a href="${path}/admin/memberListView/${row.id}">${row.id}</a></td>
  <td>${row.nickName}</td>
  <td>${row.name}</td>
  ...
  <td>${row.career}</td>
  <td><a id="#withdrawal_a" href="${path}/admin/onclickWithdrawal/${row.id}" onclick="return confirm('${row.name}님을 탈퇴처리 하시겠습니까?')">탈퇴처리</a></td>
 </tr>
</c:forEach>
</table>
...
<div id="search_div">
<form name="search_form" method="post" action="${path}/admin/memberSearch.do">
